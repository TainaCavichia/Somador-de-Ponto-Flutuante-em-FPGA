...
begin
    ...
    -- deslocar a fracao de acordo com os zeros contados
    with leado select
        sum_norm <=
    ...
    -- normalizar com condicoes especiais
    process(sum, sum_norm, expb, leado)
    begin
        if sum(8) = '1' then
            expn  <= expb + 1;
            fracn <= sum(8 downto 1);        -- houve carry-out: desloca p/ direita
